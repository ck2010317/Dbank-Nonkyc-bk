...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (params.id.startsWith("demo-card-")) {
      console.log("[v0] Demo mode: Unfreezing mock card", params.id)
    ...
    const response = await fetch(`${API_BASE_URL}/cards/${params.id}/unfreeze`, {
      method: "POST",
      headers: {
        "X-API-Key": API_KEY,
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({ message: "Failed to unfreeze card" }))
      return NextResponse.json({ error: error.message || "Failed to unfreeze card" }, { status: response.status })
    }

    const data = await response.json()
    return NextResponse.json(data)
  } catch (error) {
    console.error("[v0] Error unfreezing card:", error)
